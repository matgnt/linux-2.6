...
static int isp1362_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	ports = min(MAX_ROOT_PORTS, ports);
...
{
	seq_printf(s, "%-15s %04x%s%s%s%s%s%s%s\n", label, mask,
		   mask & HCuPINT_CLKRDY ? " clkrdy" : "",
		   mask & HCuPINT_SUSP ? " susp" : "",
		   mask & HCuPINT_OPR ? " opr" : "",
		   mask & HCuPINT_OTG ? " otg" : "",
...
static void dump_regs(struct seq_file *s, struct isp1362_hcd *isp1362_hcd)
{
	...
	seq_printf(s, "\n");
	seq_printf(s, "OTGCONTROL [%02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGCONTROL),
		   isp1362_read_reg16(isp1362_hcd, OTGCONTROL));
	seq_printf(s, "OTGSTATUS  [%02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGSTATUS),
		   isp1362_read_reg16(isp1362_hcd, OTGSTATUS));
	seq_printf(s, "OTGINT     [%02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGINT),
		   isp1362_read_reg16(isp1362_hcd, OTGINT));
	seq_printf(s, "OTGINTENB  [%02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGINTENB),
		   isp1362_read_reg16(isp1362_hcd, OTGINTENB));
	seq_printf(s, "OTGTIMER   %02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGTIMER),
		   isp1362_read_reg16(isp1362_hcd, OTGTIMER));
	seq_printf(s, "OTGALTTMR  [%02x]     %04x\n", ISP1362_REG_NO(ISP1362_REG_OTGALTTMR),
		   isp1362_read_reg16(isp1362_hcd, OTGALTTMR));
}
...
static int isp1362_mem_config(struct usb_hcd *hcd)
{
	...
	dev_info(hcd->self.controller, "  INTL: %4d * (%3ld+8):  %4d @ $%04x\n",
		 ISP1362_INTL_BUFFERS, (long) (intl_blksize - PTD_HEADER_SIZE),
		 intl_size, istl_size);
	dev_info(hcd->self.controller, "  ATL : %4d * (%3ld+8):  %4d @ $%04x\n",
		 atl_buffers, (long) ( atl_blksize - PTD_HEADER_SIZE),
...
static int isp1362_hc_reset(struct usb_hcd *hcd)
{
	...
	spin_unlock_irqrestore(&isp1362_hcd->lock, flags);
	if (!clkrdy) {
		ERR("%s Clock not ready after %lums\n", hcd_name, timeout);
		ret = -ENODEV;
	}
	return ret;
}
...
static void isp1362_hc_stop(struct usb_hcd *hcd)
{
	...
	if (board && board->is_otg)
	{
		// diasble and clear any OTG interrupt
		isp1362_write_reg16(isp1362_hcd, OTGINTENB, 0);
...
static int isp1362_chip_test(struct isp1362_hcd *isp1362_hcd)
{
	...
	printk("%s: Start chipset built-in memory test\n", hcd_name);

#if 0
	uint8_t *buf1 = kmalloc(ISP1362_BUF_SIZE, GFP_KERNEL);
	uint8_t *buf2 = kmalloc(ISP1362_BUF_SIZE, GFP_KERNEL);

	if (buf1 && buf2)
	{
		printk("buf1=%p, buf2=%p\n", buf1, buf2);

		spin_lock_irqsave(&isp1362_hcd->lock, flags);
		isp1362_read_buffer(isp1362_hcd, buf1, 0, ISP1362_BUF_SIZE);
		isp1362_read_buffer(isp1362_hcd, buf2, 0, ISP1362_BUF_SIZE);
		spin_unlock_irqrestore(&isp1362_hcd->lock, flags);

		printk("Buf 1\n");
		dump_data(buf1, ISP1362_BUF_SIZE);

		printk("Buf 2\n");
		dump_data(buf2, ISP1362_BUF_SIZE);
	}
...
static int isp1362_hc_start(struct usb_hcd *hcd)
{
	...
	if (board->remote_wakeup_connected == 0) {
		hwcfg |= HCHWCFG_DISABLE_SUSPEND;
	}
	...
	if (board && board->is_otg)
		isp1362_hcd->irqenb |= HCuPINT_OTG;
...
static int __init isp1362_probe(struct platform_device *pdev)
{
	...
	retval = usb_add_hcd(
		hcd,
		irq,
		  (isp1362_hcd->board->int_act_high ? IRQF_TRIGGER_HIGH : IRQF_TRIGGER_LOW)
